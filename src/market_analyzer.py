...
"""
        ...
        try:
            ...
            if stats:
                ...
                logger.info(
                    ...
                    "limit_up=%s limit_down=%s amount=%.0f亿 hist_amount=%s",
                    self._log_context(),
                    overview.up_count,
                    overview.down_count,
                    overview.flat_count,
    ...
    def _build_stats_block(self, overview: MarketOverview) -> str:
        ...
        lines = [
            ...
            f"| 两市成交额 | {overview.total_amount:.0f} 亿 | {self._describe_turnover(overview.total_amount)}， 前5日成交额 **{overview.hist_amount}** (单位:亿) |",
        ]
        return "\n".join(lines)
...
（列出需要关注的风险点；最后补充“建议仅供参考，不构成投资建议”。）"""
    ...
    def _build_review_prompt(self, overview: MarketOverview, news: List) -> str:
        ...
        # 资金流向信息
        top_fundflow_text = ", ".join([
            f"{s['name']}({s['net_flow']:.2f}亿,{s['change_pct']:+.2f}%)"
            for s in overview.top_fundflow_sectors
        ])
        bottom_fundflow_text = ", ".join([
            f"{s['name']}({s['net_flow']:.2f}亿,{s['change_pct']:+.2f}%)"
            for s in overview.bottom_fundflow_sectors
        ])
